<script>
import { ref } from 'vue'
import { SliderCaptcha, ClickCaptcha } from '@captcha-pro/vue2'
...
<template>
  <section class="demo-section">
    ...
    <div class="info-box">
      ...
      <ul>
        <li><strong>{{ t('弹窗焦点陷阱', 'Popup Focus Trap') }} <span class="new-badge">v2.3.0</span>:</strong>
          <ul>
        ...
        <li><strong>{{ t('键盘操作', 'Keyboard Operation') }}:</strong>
          <ul>
            ...
            <li><code>←/→</code> - {{ t('调整滑块位置（滑块验证码）', 'Adjust slider position (Slider Captcha)') }}</li>
          </ul>
        </li>
        <li><strong>{{ t('屏幕阅读器', 'Screen Reader') }}:</strong>
          <ul>
            ...
            <li>{{ t('验证码说明文本可访问', 'Captcha instructions are accessible') }}</li>
          </ul>
        </li>
        <li><strong>{{ t('触摸目标', 'Touch Targets') }}:</strong>
          <ul>
            <li>{{ t('刷新按钮触摸区域 44x44 像素', 'Refresh button touch target 44x44 pixels') }}</li>
            <li>{{ t('点选标记触摸区域 24x24 像素', 'Click marker touch target 24x24 pixels') }}</li>
          </ul>
        </li>
      </ul>
    </div>
    ...
    <div class="demo-grid">
      <div class="demo-item">
        ...
        <p class="hint">{{ t('提示：使用 Tab 聚焦滑块，用 ←/→ 键调整位置', 'Tip: Use Tab to focus slider, ←/→ keys to adjust position') }}</p>
      </div>
      ...
      <div class="demo-item">
        ...
        <p class="hint">{{ t('提示：屏幕阅读器会播报点击的字符序号', 'Tip: Screen reader announces clicked character numbers') }}</p>
      </div>
    </div>
