...
import { Canvas, Text, View, MovableArea, MovableView, CoverView, CoverImage } from '@tarojs/components'
...
const SliderCaptcha: FC<SliderCaptchaProps> = ({
  ...
  const generateCaptcha = useCallback(() => {
    ...
    const newTargetX = Math.floor(
      Math.random() * (canvasWidth - actualSliderWidth - 20) + actualSliderWidth + 10
    )
    const newSliderY = Math.floor(
      Math.random() * (canvasHeight - actualSliderHeight - 20) + 10
    )
  ...
  return (
    <View className="captcha-container">
      ...
      >
        ...
        {/* Slider block overlay - use cover-view to overlay canvas */}
        {isReady && (
          <CoverView
          ...
          >
            <CoverView className="slider-block-inner">
              <Text className="slider-block-icon">≫</Text>
            </CoverView>
          </CoverView>
        )}

        {/* Refresh button - use cover-view */}
        {showRefresh && (
          <CoverView className="refresh-btn" onClick={refresh}>
            <Text className="refresh-icon">⟳</Text>
          </CoverView>
        )}

        {/* Status overlay - use cover-view */}
        {status && (
          <CoverView className={`status-overlay ${status}`}>
            <Text className="status-text">{status === 'success' ? '验证成功' : '验证失败'}</Text>
          </CoverView>
        )}
      </View>

      {/* Slider bar - outside canvas, can use normal View */}
      <View
