...
export default {
  ...
  methods: {
    toggleTheme() {
      ...
      try {
        Taro.setStorageSync('cp-theme', this.isDark ? 'dark' : 'light')
      } catch (e) {
        // ignore
      }
    }
...
<template>
  <view class="index" :class="isDark ? 'cp-dark' : 'cp-light'">
    <view class="title">Captcha Pro</view>
    <view class="subtitle">Taro + Vue 2 示例</view>
    ...
    <navigator url="/pages/popup/index" class="card">
      ...
      <text class="card-desc">弹窗形式调用滑块/点击验证</text>
    </navigator>
  </view>
</template>
