...
<template>
  <section class="demo-section">
    ...
    <div class="info-box">
      ...
      <p>{{ t('通过 props 自定义验证码组件的行为和外观', 'Customize captcha component behavior and appearance via props') }}</p>
    </div>
    ...
    <div class="config-panel">
      ...
      <div class="config-grid">
        <div class="config-item">
          <label>
            <span class="config-label">{{ t('验证精度', 'Verification Precision') }}</span>
            <span class="config-desc">{{ t('像素级精度容差（1-20）', 'Pixel-level precision tolerance (1-20)') }}</span>
          </label>
        ...
        <div class="config-item">
          <label>
            <span class="config-label">{{ t('显示刷新按钮', 'Show Refresh Button') }}</span>
            <span class="config-desc">{{ t('是否显示刷新按钮', 'Whether to show refresh button') }}</span>
          </label>
          <label class="toggle">
