...
|------|------------------|------------|----------------------|
| **项目类型** | 前端库 + 后端服务（小程序仅后端模式） | 后端 + 前端全栈方案 | 云服务 SaaS |
...
| **自定义图片** | ✅ 支持 | ✅ 支持 | ❌ 不支持 |
| **离线可用** | ✅ 支持（Web 前端模式）/ ❌ 不支持（小程序） | ❌ 不支持 | ❌ 不支持 |
...
|------|------------------|------------|-----------|
| **前端验证** | ✅ 内置（Web）/ ❌ 不支持（小程序，仅后端验证） | ❌ 不支持（需后端） | ❌ 不支持 |
| **后端验证** | ✅ 可选支持 | ✅ 内置 | ✅ 云端托管 |
...
三者各有优势，选择应根据：
- **安全要求**：腾讯验证码 ≈ AJ-Captcha ≈ Captcha-Pro (后端示例/小程序) > Captcha-Pro (前端模式，仅Web)
