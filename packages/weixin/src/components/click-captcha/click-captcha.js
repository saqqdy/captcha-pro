  ...
  methods: {
    ...
    onAreaTap(e) {
      ...
      query.select('.captcha-area').boundingClientRect((rect) => {
        ...
        const x = this.pxToRpx(clientX - rect.left)
        const y = this.pxToRpx(clientY - rect.top)
    ...
    async verify() {
      ...
      try {
        const pts = this.data.clickMarkers.map(m => ({ x: this.rpxToPx(m.x), y: this.rpxToPx(m.y) }))
