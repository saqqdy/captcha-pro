...
export function useSliderCaptcha(options: UseSliderCaptchaOptions): {
  ...
  const instance = ref<SliderCaptchaInstance | null>(null)
  ...
  const init = (): void => {
    ...
    const coreOptions: SliderCaptchaOptions = {
      el: container,
      width: options.width,
    ...
    instance.value = new SliderCaptchaCore(coreOptions) as SliderCaptchaInstance
    isReady.value = true
  }
